...
import { titleShareCopy, type TitleShare, type TitleSummary } from "@cinima/shared";
...
const payload = ref<TitleShare | null>(null);
const gateTitle = ref<TitleSummary | null>(null);

const handle = computed(() => String(route.params.handle || ""));
const mediaType = computed(() => String(route.params.mediaType || ""));
const tmdbId = computed(() => String(route.params.tmdbId || ""));
...
const loadShare = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `${apiBase}/api/public/${encodeURIComponent(handle.value)}/t/${mediaType.value}/${tmdbId.value}`
    );
...
watch(
  () => [handle.value, mediaType.value, tmdbId.value].join("/"),
  () => {
    loadShare();
  },
  { immediate: true }
